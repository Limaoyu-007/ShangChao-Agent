...
> 已实现后台单次颁旨、皇帝与一名大臣的并行议事、公开记录保存和后台读取反馈。定时自治、朝臣调查工具、任务执行与图形界面仍待建设。[查看开发进度](#progress) · [直接体验](#quickstart)
...
每位朝臣可以拥有不同的人设、专业关注点，以及各自的任务状态、工具权限和独立记忆。用户的人生最高准则只提供给皇帝；朝臣从不同视角献策，由皇帝进行最终权衡。当前仅实现一名大臣的人设与议事能力，尚无独立工具和长期记忆。
